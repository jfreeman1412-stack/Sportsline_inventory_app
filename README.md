...
- **System settings**: Managers/Owners can edit alert configuration from **System Settings** (`/settings/system`). Toggle email alerts on/off, update SMTP credentials, and change the price-spike threshold (%) or the low-stock call-to-action message so emails point people at your preferred supplier. The existing `Settings` link still manages user alert opt-ins.
